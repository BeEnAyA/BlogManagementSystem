...
exports.renderSingleBlog = async (req, res) => {
  ...
  const [blog] = await db.sequelize.query(
    "SELECT blogs.id,blogs.title,blogs.description,blogs.createdAt,blogs.image,users.fullname FROM blogs JOIN users ON blogs.userId=users.id WHERE blogs.id=?",
    {
